...
@section('content')
<form method="get" action="{{ route('books.index') }}" class="space-x-4 flex items-center mb-4">
...
<div class="flex items-center justify-center gap-4 p-2 rounded-lg mb-4">
    @php
        $filters = [
            '' => 'Latest',
...
<ul class="grid grid-cols-1 gap-4 items-center justify-center w-full mx-auto max-w-2xl">
    ...
    <a href="{{ route('books.show', ['book' => $book]) }}" class="col-span-1 w-full bg-white rounded-md p-4 shadow-md flex flex-col hover:shadow-lg transition-shadow duration-300">
